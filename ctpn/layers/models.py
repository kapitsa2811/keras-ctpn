...
def ctpn_net(config, stage='train'):
    ...
    '''
      measure change here is nw takes 4 input instead of 1 
    '''
...
def ctpn(base_features, num_anchors, rnn_units=128, fc_units=512):
    ...
    # 沿着宽度方式做  Do it along the width rnn
    ...
    # 中心点垂直坐标和高度回归 Center point vertical coordinate and height 
    ...
    # 侧边精调(只需要预测x偏移即可) Side fine adjustment (just need to predict x offset)
...
def compile(keras_model, config, loss_names=[]):
    """
    编译模型，增加损失函数，L2正则化以  compile the model, increase the loss function, L2 regularize to
    :param keras_model:
    :param config:
    ...
    # 增加损失函数，首先清除之前的，防止重复 Increase the loss function, first clear the previous one, prevent duplication
    keras_model._losses = []
    ...
    # 增加L2正则化 Increase L2 regularization
    # 跳过批标准化层的 gamma 和 beta 权重  Skip the gamma and beta weights of the batch normalization layer
    reg_losses = [
    ...
    # 为每个损失函数增加度量  Add a measure for each loss function
    for name in loss_names:
